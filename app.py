...
st.title("Paliopoly – Fully Automatic")
...
BOARD = [
    ...
    ("Go to Jail",go2jail),                # 18  ← corner
    ("Chappa Chest",chest),                # 19  ← second chest
...
# ====================== CARDS – YOUR EXACT WORDING ======================
CHEST_CARDS = {
    2: "Advance to GO → +300g",
    3: "JAIL do not pass GO → go to Jail",
    4: "Everyone pays you 50g",
    5: "Pay 100g",
    6: "Collect 100g",
    7: "Back 3 spaces",
    8: "Forward 3 spaces",
    9: "Pay poorest player 100g",
    10: "Go to nearest Travel Point",
    11: "Pay 150g",
    12: "Collect 200g"
}

CHANCE_CARDS = {
    2: "Free Parking → move there",
    3: "Get Out of Jail Free Card!",
    4: "Give 50g to each player",
    5: "Pay 200g",
    6: "Get 150g",
    7: "Move to next Utility",
    8: "Move to next main property",
    9: "All give you 100g",
    10: "Go to nearest property owned by anyone",
    11: "Pay 100g",
    12: "Get 200g"
}
...
if 'players' not in st.session_state:
    ...
    names = st.text_input("Names (comma separated)", "Reth,Tish,Hodari,Ashura")
    ...
    if dice.isdigit():
        ...
        if 2 <= roll <= 12:
            ...
            pos[cur] = new_pos
            space_name, space_type, *data = BOARD[new_pos]

            # Pass GO?
            if new_pos < old_pos or (old_pos == 23 and new_pos == 0):
                cash[cur] += 300
                st.balloons()
            ...
            # Go to Jail corner
                ...
                pos[cur] = 6
                jail[cur] = True
                message = "Go to Jail! → moved to Jail"
            ...
            # Chappa Chest
                ...
                elif roll == 4:  # everyone pays you 50
                    for pl in p:
                        if pl != cur and not jail[pl]:
                            cash[pl] -= 50
                            cash[cur] += 50
                ...
                elif roll == 9:  # pay poorest
                    alive = [pl for pl in p if cash[pl] >= 0]
                    poorest = min(alive, key=lambda x: cash[x])
                    cash[cur] -= 100; cash[poorest] += 100
                elif roll == 10:  # nearest Travel Point
            ...
            # Chapaa Chance
                ...
                elif roll == 7:  # next utility
                    for i in range(new_pos+1, new_pos+25):
                        if BOARD[i%24][1] == "util":
                            pos[cur] = i%24; break
                elif roll == 8:  # next main property
                    for i in range(new_pos+1, new_pos+25):
                        if BOARD[i%24][1] == "prop":
                            pos[cur] = i%24; break
                elif roll == 9:  # all give you 100
                    for pl in p:
                        if pl != cur and not jail[pl]:
                            cash[pl] -= 100
                            cash[cur] += 100
                elif roll == 10: # nearest owned by anyone
                    for i in range(new_pos+1, new_pos+25):
                        idx = i%24
            ...
            # AUTO RENT
            elif space_type in ("prop","rail","util") and owner[new_pos] and owner[new_pos] != cur:
                ...
                if space_type == "prop":
                    ...
                    rent = data[4] if full else data[3]
            ...
            if space_type in ("prop","rail","util") and owner[new_pos] is None:
                price = data[0]
                if st.button(f"BUY {space_name} for {price}g"):
                    if cash[cur] >= price:
                        cash[cur] -= price
                        owner[new_pos] = cur
                        st.success(f"Bought {space_name}!")
                    else:
    ...
    # Quick buttons
    colq = st.columns(4)
    with colq[0]: if st.button("Renown Tax 100g"): cash[cur] -= 100
    with colq[1]: if st.button("Maji Tax 200g"): cash[cur] -= 200
    with colq[2]: if st.button("Out of Jail (100g)"): 
