...
CHEST_CARDS_LIST = [
    ...
    ("Eshe made Kenli enforce the land tax. Pay 100g", lambda p,ss,roll: (ss['cash'].__setitem__(p, ss['cash'][p]-100), ss['free_parking_pot'] +=100)),
    ("Collect 100g from Subira for helping the order", lambda p,ss,roll: ss['cash'].__setitem__(p, ss['cash'][p]+100)),
    ("Ogupu drags you into a whirlpool and moves you back 3 spaces", lambda p,ss,roll: (old:=ss['position'][p], ss['position'].__setitem__(p, (old-3) % len(BOARD)))),
    ("Bluebristle Muujin pushes you forward 3 spaces", lambda p,ss,roll: (old:=ss['position'][p], ss['position'].__setitem__(p, (old+3) % len(BOARD)), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ...
    ("You followed a Peki to the next Travel Point", lambda p,ss,roll: (old:=ss['position'][p], ss['position'].__setitem__(p, min([4,9,16,20], key=lambda x:(x-old)%len(BOARD))), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ("Tish has new furniture, pay 150g", lambda p,ss,roll: (ss['cash'].__setitem__(p, ss['cash'][p]-150), ss['free_parking_pot'] +=150)),
...
CHANCE_CARDS_LIST = [
    ...
    ("Caught in the restricted section, pay Caleri 200g", lambda p,ss,roll: (ss['cash'].__setitem__(p, ss['cash'][p]-200), ss['free_parking_pot'] +=200)),
    ("Collect 150g for promoting Jels new wardrobe", lambda p,ss,roll: ss['cash'].__setitem__(p, ss['cash'][p]+150)),
    ("Follow a flutterfox to the next shrub", lambda p,ss,roll: (old:=ss['position'][p], ss['position'].__setitem__(p, next((i for i in [11,17] if i>old),11)), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ("Ormuu pushes you to next main property", lambda p,ss,roll: (old:=ss['position'][p], ss['position'].__setitem__(p, next((i for i in [1,3,7,10,13,15,21,23] if i>old),1)), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ...
    ("Go and help the truffle at the nearest owned property", lambda p,ss,roll: (old:=ss['position'][p], ss['position'].__setitem__(p, min([i for i,o in ss['properties'].items() if o and BOARD[i][1] in ('prop','rail','util')], key=lambda x:(x-old)%len(BOARD), default=old)), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ("you lost the Gardners runestone, Pay 100g", lambda p,ss,roll: (ss['cash'].__setitem__(p, ss['cash'][p]-100), ss['free_parking_pot'] +=100)),
...
if not st.session_state.initialized:
    ...
    if st.button("Start Game"):
        ...
        if len(players) < 2:
            ...
            st.session_state.properties = {i: None for i in range(len(BOARD)) if BOARD[i][1] in ("prop", "rail", "util")}
...
if st.session_state.initialized:
    ...
    if cur and not ss['bankrupt'].get(cur, False):
        ...
        if not ss['rolled']:
            ...
            if ss['in_jail'].get(cur, False):
                ...
                if ss['jail_free_card'] == cur and st.button("Use Get Out of Jail Free Card"):
                    ss['in_jail'][cur] = False
                    ss['jail_turns'][cur] = 0
                    ss['jail_free_card'] = None
                    ss['last_message'] = f"{cur} used Get Out of Jail Free card."
                    st.rerun()
                ...
                if ss['in_jail'].get(cur, False):
                    if doubles:
                        ...
                        if ss['jail_turns'][cur] >= 3:
                            # On 3rd turn, if no doubles, pay 50 to get out, then move
                            if ss['cash'].get(cur,0) >= 50:
                                ss['cash'][cur] -= 50
                                ss['free_parking_pot'] += 50
                                ss['in_jail'][cur] = False
                                ss['jail_turns'][cur] = 0
                                ss['last_message'] = f"{cur} failed 3 times → paid 50g, released and moves {roll}."
                                # fallthrough to movement
                            else:
                                ss['last_message'] = f"{cur} failed 3 times but can't pay 50g — stays in jail!"
                                ss['rolled'] = True
    ...
    # Non-initialized state: nothing else to render beyond start UI
    pass
