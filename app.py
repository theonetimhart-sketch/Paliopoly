...
st.image("https://raw.githubusercontent.com/theonetimhart-sketch/Paliopoly/refs/heads/main/image.png",
         use_column_width=True)  # your beautiful header

st.image("https://raw.githubusercontent.com/theonetimhart-sketch/Paliopoly/refs/heads/main/image2.png",
         use_column_width=True, caption="The Board")  # the actual board
...
if 'players' not in st.session_state:
    ...
    # ====================== NORMAL TURN ======================
        ...
        total = st.text_input("Enter total dice rolled here (2–12)", value="", key="dice_total")
        if total.isdigit():
            roll = int(total)
            if 2 <= roll <= 12:
                ...
                if st.session_state.doubles_count >= 3:
                    ...
                    st.error("3 DOUBLES IN A ROW → JAIL!")
                else:
                    old = pos[cur]
                    new = (old + roll) % 24
                    pos[cur] = new
                    ...
                    if new <= old and new != 0:
                        cash[cur] += 300; st.balloons()

                    if typ == "tax":
                        cash[cur] -= BOARD[new][2]; st.info(f"Tax paid: {BOARD[new][2]}g")
                    elif typ == "go2jail":
                        pos[cur] = 6; jail[cur] = True; jail_turns[cur] = 0
                        st.session_state.doubles_count = 0; st.error("Go to Jail!")

                    # Cards end turn
                    if typ == "chest":
                        txt = CHEST.get(roll, "")
                        st.success(f"CHAPPA CHEST → {txt}")
                        # (full card effects here – same as before)
        ...
        st.info("Turn finished → Next player")
    ...
    with st.expander("Trades"):
        ...
        if st.button("Execute Deal"):
            if amount == 0 and prop == "(none)" and jailfree_trade == "(none)":
                ...
                if amount > 0: cash[giver] -= amount; cash[receiver] += amount
                if prop != "(none)":
