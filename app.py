...
# ======================
# FULL CHEST & CHANCE CARDS (11 each)
# ======================
CHEST_CARDS_LIST = [
    ...
    ("Eshe made Kenli enforce the land tax. Pay 100g",
     lambda p,ss,_: (ss.__setitem__('free_parking_pot', ss['free_parking_pot']+100), ss['cash'].__setitem__(p, ss['cash'][p]-100))),
    ...
    ("Bluebristle Muujin pushes you forward 3 spaces",
     lambda p,ss,_: (old:=ss['position'][p], ss['position'].__setitem__(p, (old+3)%len(BOARD)), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ...
    ("You followed a Peki to the next Travel Point",
     lambda p,ss,_: (old:=ss['position'][p], ss['position'].__setitem__(p, min([4,9,16,20], key=lambda x:(x-old)%len(BOARD))), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ("Tish has new furniture, pay 150g",
     lambda p,ss,_: (ss['cash'].__setitem__(p, ss['cash'][p]-150), ss.__setitem__('free_parking_pot', ss['free_parking_pot']+150))),
...
CHANCE_CARDS_LIST = [
    ...
    ("Jina found a rare artifact. Give 50g to all the humans",
     lambda p,ss,_: [ss['cash'].__setitem__(q, ss['cash'][q]+50) or ss['cash'].__setitem__(p, ss['cash'][p]-50) for q in ss['players'] if q!=p]),
    ("Caught in the restricted section, pay Caleri 200g",
     lambda p,ss,_: (ss['cash'].__setitem__(p, ss['cash'][p]-200), ss.__setitem__('free_parking_pot', ss['free_parking_pot']+200))),
    ("Collect 150g for promoting Jels new wardrobe", lambda p,ss,_: ss['cash'].__setitem__(p, ss['cash'][p]+150)),
    ("Follow a flutterfox to the next shrub",
     lambda p,ss,_: (old:=ss['position'][p], ss['position'].__setitem__(p, next((i for i in [11,17] if i>old), 11)), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ("Ormuu pushes you to next main property",
     lambda p,ss,_: (old:=ss['position'][p], ss['position'].__setitem__(p, next((i for i in [1,3,7,10,13,15,21,23] if i>old), 1)), ss['cash'].__setitem__(p, ss['cash'][p]+300) if ss['position'][p] < old else None)),
    ("Badruu gives you new fruit, everyone gives you 100g for the seeds",
     lambda p,ss,_: [ss['cash'].__setitem__(q, ss['cash'][q]-100) or ss['cash'].__setitem__(p, ss['cash'][p]+100) for q in ss['players'] if q!=p]),
    ("You brought Tamala the wrong ingredient. You have been slapped to the nearest owned property",
     lambda p,ss,_: (old:=ss['position'][p], target:=min([i for i,o in ss['properties'].items() if o and BOARD[i][1] in ('prop','rail','util')], key=lambda x:(x-old)%len(BOARD), default=old), ss['position'].__setitem__(p, target), ss['cash'].__setitem__(p, ss['cash'][p]+300) if target != old and ss['position'][p] < old else None)),
    ("you lost the Gardners runestone, Pay 100g",
     lambda p,ss,_: (ss['cash'].__setitem__(p, ss['cash'][p]-100), ss.__setitem__('free_parking_pot', ss['free_parking_pot']+100))),
    ("Reth just started selling beanburgers and flowtato fries, he pays you 200g",
     lambda p,ss,_: ss['cash'].__setitem__(p, ss['cash'][p]+200))
]
...
if 'initialized' not in st.session_state:
    st.session_state.update({
        ...
        'easter_eggs_used': {p:{'Chilled Dude': False, 'shortee': False} for p in st.session_state.players},
    })
...
if not ss.rolled:
    ...
    if st.button("Confirm Roll", type="primary"):
        # Jail logic omitted for brevity (unchanged)

        old_pos = ss.position[cur]
        new_pos = (old_pos + roll) % len(BOARD)
        passed_go = (old_pos + roll >= len(BOARD)) or new_pos == 0
        if passed_go: ss.cash[cur] += 300; st.balloons()
        ss.position[cur] = new_pos; ss.landed = new_pos; ss.starting_square = BOARD[old_pos][0]; ss.rolled = True
        ...
        def land_on(pos, depth=0):
            ...
            if typ=="tax":
                ...
                if owner and owner != cur:
                    if typ=="prop":
                        base_rent=sq[3]; color=sq[5]; full_set=all(ss.properties.get(i)==owner for i in COLOR_GROUPS[color]); rent=base_rent*2 if full_set else base_rent
                    elif typ=="rail": owned=sum(1 for i,o in ss.properties.items() if o==owner and BOARD[i][1]=="rail"); rent=40*(2**(owned-1))
                    else: owned=sum(1 for i,o in ss.properties.items() if o==owner and BOARD[i][1]=="util"); rent=roll*(10 if owned==2 else 4)
...
# ======================
# EASTER EGGS OUTSIDE RECURSION
# ======================
pos = ss.position[cur]
occupants = [p for p in ss.players if ss.position[p]==pos and p!=cur]

# Chilled Dude auto bonus
if "Chilled Dude" in occupants and not ss.easter_eggs_used[cur]['Chilled Dude']:
    ss.cash[cur]+=10; ss.easter_eggs_used[cur]['Chilled Dude']=True
    st.success("Hanging with Chilled Dude — collect 10g for finding the creator!")

# ShorTee bonus
if "lilshrtchit.ttv" in occupants and not ss.easter_eggs_used[cur]['shortee']:
    if st.button("Are you following ShorTee on stream?", key="shortee_egg"):
        ss.cash[cur]+=10; ss.easter_eggs_used[cur]['shortee']=True
        st.success("+10g for following ShorTee!")
...
with st.expander("Players & Cash", expanded=True):
    cols = st.columns(len(ss['players']))
    for i, p in enumerate(ss['players']):
        jail = " (JAILED)" if ss['in_jail'][p] else ""
        bank = " (BANKRUPT)" if ss['bankrupt'][p] else ""
        at = f"at {BOARD[ss['position'][p]][0]}"
        cols[i].markdown(f"**{p}**{jail}{bank}\n{ss['cash'][p]}g\n{at}")

c1, c2, c3, c4 = st.columns([2,2,2,3])
c1.markdown(f"**Turn: {cur}** {'(JAILED)' if ss['in_jail'][cur] else ''}")
c2.markdown(f"**Gold:** {ss['cash'][cur]}g")
c3.markdown(f"**Free Parking Pot:** {ss['free_parking_pot']}g")
if ss['rolled']:
    c4.success(f"Started on: **{ss['starting_square']}**")
...
if not ss['rolled']:
    ...
    if st.button("Confirm Roll", type="primary"):
        ...
        if not ss['in_jail'][cur]:
            ...
            # ======================
            # Land On Square Logic (with Easter Eggs)
            # ======================
            def land_on(pos, depth=0):
                if depth > 6: return " [card loop stopped]"
                sq = BOARD[pos]
                ...
                # Player-on-player Easter Eggs
                if typ in ("prop","rail","util"):
                    occupants = [p for p in ss['players'] if ss['position'][p]==pos and p!=cur]
                    for occ in occupants:
                        # Chilled Dude auto bonus
                        if occ=="Chilled Dude" and not ss['easter_eggs_used'][cur]['Chilled Dude']:
                            ss['cash'][cur] += 10
                            ss['easter_eggs_used'][cur]['Chilled Dude'] = True
                            msg.append("Hanging with Chilled Dude — collect 10g for finding the creator!")
                        # ShorTee bonus
                        if occ=="lilshrtchit.ttv" and not ss['easter_eggs_used'][cur]['shortee']:
                            ss['last_message'] = "Landed on ShorTee"
                            if st.button("Are you following on stream?"):
                                ss['cash'][cur] += 10
                                ss['easter_eggs_used'][cur]['shortee'] = True
                                ss['last_message'] += " — Thanks for following! +10g"
                            else:
                                ss['easter_eggs_used'][cur]['shortee'] = True

                # Standard square effects
                if typ == "tax":
                    amt = sq[2]; ss['cash'][cur] -= amt; ss['free_parking_pot'] += amt; msg.append(f"Paid {amt}g tax")
                elif typ == "free":
...
# Buy Property
# ======================
if ss['rolled'] and ss['landed'] is not None and not ss['in_jail'][cur]:
    sq = BOARD[ss['landed']]
...
if ss['trade_mode']:
    ...
    others = [p for p in ss['players'] if p != cur and not ss['bankrupt'][p]]
    if not others:
