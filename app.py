...
st.markdown("**Fun Easter eggs, friendly multiplayer board game inside Palia!**")
...
if not st.session_state['splash_done']:
    ...
    # Password input for Chilled Dude presence
    password = st.text_input("Enter host password if Chilled Dude is not present", type="password")
    if st.button("Confirm"):
        if password == "TJediTim":
            st.session_state['splash_done'] = True
            st.success("Password accepted — continue!")
        else:
            st.error("Incorrect password, please enter the correct host password.")
    st.stop()  # Stop until splash is confirmed

# ======================
# BOARD IMAGES IN-GAME
# ======================
...
CHANCE_CARDS_LIST = [
    ...
    ("Follow a flutterfox to the next utility", lambda p, ss, roll: ss['position'].__setitem__(p, next((i for i in [13,15,22,24] if i>ss['position'][p]),13))),
    ...
    ("you lost the Gardners runestone, Pay 100g to Einar and Hekla to sort it", lambda p, ss, roll: ss['cash'].__setitem__(p, ss['cash'][p]-100)),
    ("Reth just started selling beanburgers and flowtato fries, he pays you 200g for the idea", lambda p, ss, roll: ss['cash'].__setitem__(p, ss['cash'][p]+200))
]
...
if not st.session_state.initialized:
    ...
    # Password check for host (Chilled Dude) if not present
    player_names_input = st.text_input("Enter player names (comma separated)", "Chilled Dude, lilshrtchit, Player3")
    host_present = any("chilled dude" in n.lower() for n in [x.strip() for x in player_names_input.split(",")])
    password_correct = False

    if not host_present:
        pw_input = st.text_input("Chilled Dude is not present. Enter host password to continue:", type="password")
        if pw_input == "TJediTim":
            password_correct = True
            st.success("Host verified. Game can continue without Chilled Dude present.")
        elif pw_input:
            st.error("Incorrect password!")

    if st.button("Start Game"):
        if host_present or password_correct:
            players = [n.strip() for n in player_names_input.split(",") if n.strip()]
            if len(players) < 2:
                ...
                ss.easter_eggs_claimed = {p: {"chilled dude": False, "lilshrtchit": False} for p in players}
                ss.initialized = True
...
if ss.bankrupt.get(cur, False):
    nxt_idx = next_active_idx(cur_idx)
    if nxt_idx is not None:
        ss.current_idx = nxt_idx
        st.experimental_rerun()
    else:
        check_game_end()
        st.stop()
...
if not ss.rolled:
    ...
    if st.button("Confirm Roll", type="primary"):
        ...
        if ss.in_jail.get(cur, False):
            if doubles:
                ...
                if ss.jail_turns[cur] >= 3:
                    ss.in_jail[cur] = False
                    ss.jail_turns[cur] = 0
                    ss.last_message = f"{cur} failed 3 turns in jail → released and moves {roll}."
                else:
        ...
        # Handle doubles streak (3 doubles → jail)
        if doubles:
            ss.doubles_streak += 1
