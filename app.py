...
if not st.session_state.passed_splash:
    ...
    st.markdown("### Hi ShorTee / lilshrtchit, thanks for hosting!")
    st.write("Make sure everyone playing is watching at [Twitch](https://www.twitch.tv/lilshrtchit)")
    ...
    if any(p.lower() == "chilled dude" for p in tentative_players):
        ...
        if st.button("Continue", key="continue_with_chilled"):
            st.session_state.players = tentative_players
            st.session_state.passed_splash = True
            st.experimental_rerun()
    else:
        pwd = st.text_input("Chilled Dude isn't here? Enter his Discord name to continue:", type="password", key="pwd_input")
        if st.button("Continue", key="continue_password"):
            if pwd == "TJediTim":
                st.session_state.players = tentative_players
                st.session_state.passed_splash = True
    ...
    st.stop()  # halt until splash accepted
...
CHANCE_CARDS_LIST = [
    ...
    ("Caught in restricted section, pay Caleri 200g", lambda p, ss, roll: ss['cash'].__setitem__(p, ss['cash'][p]-200)),
    ...
    ("Badruu gives you new fruit, everyone pays 100g for seeds", lambda p, ss, roll: [ss['cash'].__setitem__(q, ss['cash'][q]-100) or ss['cash'].__setitem__(p, ss['cash'][p]+100) for q in ss['players'] if q!=p]),
    ...
    ("Reth just started selling beanburgers, he pays you 200g", lambda p, ss, roll: ss['cash'].__setitem__(p, ss['cash'][p]+200))
]
...
# Initialize per-player Easter eggs
...
# ======================
if not st.session_state.initialized or not st.session_state.get('players'):
    ...
    st.session_state.splash_players_input = st.text_input(
        "Confirm player names (comma separated):", st.session_state.splash_players_input
    )
    tentative_players = [n.strip() for n in st.session_state.splash_players_input.split(",") if n.strip()]

    # Check for Chilled Dude presence
    if "Chilled Dude" in [p.lower() for p in tentative_players]:
        if st.button("Start Game"):
            st.session_state.players = tentative_players
            st.session_state.initialized = True
            st.rerun()
    else:
        pwd = st.text_input("Chilled Dude isn't present. Enter host password to continue:", type="password")
...
with st.expander("Players and status", expanded=True):
...
if not ss['rolled']:
    ...
    if st.button("Confirm Roll", key=f"confirm_{cur}"):
        ...
        if not ss['in_jail'].get(cur, False):
            ...
            # Process landing
            landing_msg = f"Landed on {BOARD[new_pos][0]}."
            ...
            # Combine messages
            ss['last_message'] = f"{landing_msg} {go_msg} {' '.join(easter_msg)}"
