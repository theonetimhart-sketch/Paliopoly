...
if 'initialized' not in st.session_state:
    st.session_state.update({
        ...
        'show_twitch_question': False,
        'twitch_bonus_asked': False,
    })
ss = st.session_state
...
if not ss.rolled:
    ...
    if st.button("Confirm Roll", type="primary"):
        ss.show_twitch_question = False
        ss.twitch_bonus_asked = False
        if ss.in_jail.get(cur):
            if doubles:
            ...
            def land_on(pos, depth=0):
                ...
                if typ == "tax":
                    ...
                    if owner and owner != cur:
                        ...
                        if owner == "Chilled Dude":
                            ss.cash[cur] += 10
                            msg.append("Hanging with Chilled Dude, collect 10g for finding the creator!")
                        if owner == "lilshrtchit.ttv":
                            ss.show_twitch_question = True
                elif typ == "chest":
    ...
    if sq[1] in ("prop","rail","util") and ss.properties.get(ss.landed) is None:
        if st.button(f"Buy {sq[0]} for {sq[2]}g?", key=f"buy_{ss.landed}"):
            if ss.cash[cur] >= sq[2]:
                ...
                st.rerun()
# Twitch follow bonus for lilshrtchit.ttv
if ss.rolled and ss.get('show_twitch_question', False) and not ss.get('twitch_bonus_asked', False):
    st.info("Are you following on Twitch?")
    col_yes, col_no = st.columns(2)
    if col_yes.button("Yes!"):
        ss.cash[cur] += 10
        ss.last_message += " Following on Twitch! +10g bonus!"
        ss.twitch_bonus_asked = True
        st.rerun()
    if col_no.button("No"):
        ss.last_message += " Not following, no bonus."
        ss.twitch_bonus_asked = True
        st.rerun()
# Confirm next player
if ss.rolled:
