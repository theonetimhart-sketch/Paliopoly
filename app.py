...
if 'players' not in st.session_state:
    ...
    if st.button("Start Game"):
        ...
        if len(pl) < 2: st.error("Need 2+ players")
            ...
            st.session_state.jailfree_owner = None  # Only ONE card in game
    ...
    with col3: st.button("Next player", on_click=lambda: st.session_state.update(current=(st.session_state.current+1)%len(p), rolled=False))
    ...
    if jail[cur]:
        ...
        # Pay 100g
        if jc1.button("Pay 100g") and cash[cur] >= 100:
            cash[cur] -= 100
            jail[cur] = False
            jail_turns[cur] = 0
            st.success("Paid 100g → out!")

        # Use card (if you own it)
        if jc2.button("Use Jail Free Card") and jailfree_owner == cur:
            st.session_state.jailfree_owner = None
            jail[cur] = False
            jail_turns[cur] = 0
            st.success("Used Get Out of Jail Free!")

        # Roll doubles?
        if jc3.button("Rolled Doubles → YES"):
            jail[cur] = False
            jail_turns[cur] = 0
            st.success("DOUBLES! You're free!")
            # Move normally next turn

        if jc4.button("No Doubles"):
            jail_turns[cur] += 1
    ...
    # ====================== NORMAL TURN ======================
        ...
        if not st.session_state.rolled:
            ...
            dice = ""
            st.info("Turn done → Next player")

        if dice.isdigit():
            roll = int(dice)
            if 2 <= roll <= 12 and not st.session_state.rolled:
                ...
                # CHEST
                    ...
                    if roll == 2: pos[cur] = 0; cash[cur] += 300
                        ...
                        for pl in p:
                            if pl != cur: cash[pl] -= 50; cash[cur] += 50
                ...
                # CHANCE
                    ...
                    if roll == 2: pos[cur] = 12
                    elif roll == 3 and jailfree_owner is None:  # Only one card allowed
                        st.session_state.jailfree_owner = cur
                        st.success("You got the Get Out of Jail Free card!")
                    elif roll == 4:
                        for pl in p:
                            if pl != cur: cash[cur] -= 50; cash[pl] += 50
                        ...
                        for pl in p:
                            if pl != cur: cash[pl] -= 100; cash[cur] += 100
                    elif roll == 10:
                        for i in range(new + 1, new + 25):
                            idx = i % 24
    ...
    # ==================== TRADE (includes Jail Free card) ====================
    with st.expander("Trade – Cash, Properties & Jail Free Card"):
        giver = st.selectbox("Giver", p, key="tg")
        receiver = st.selectbox("Receiver", [x for x in p if x != giver], key="tr")
        amount = st.number_input("Cash (0=none)", min_value=0, value=0, step=10, key="tcash")
        owned_props = [BOARD[i][0] for i in range(24) if owner[i] == giver]
        prop = st.selectbox("Property (optional)", ["(none)"] + owned_props, key="tprop")

        # Jail Free Card trade option
        jailfree_option = "(none)"
        if jailfree_owner == giver:
            jailfree_option = "Get Out of Jail Free Card"

        jailfree_trade = st.selectbox("Jail Free Card", ["(none)", jailfree_option], key="tjail")

        if st.button("Execute Deal"):
            valid = False
            if amount > 0 or prop != "(none)" or jailfree_trade != "(none)":
                valid = True
    ...
    for pl in p:
        ...
        jailfree_text = " (has Jail Free Card)" if jailfree_owner == pl else ""
